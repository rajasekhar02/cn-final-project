...
// NS_OBJECT_ENSURE_REGISTERED(ERGCPktHashTable);

ERGCPktHashTable::ERGCPktHashTable()
{
...
{
	NS_LOG_FUNCTION(this);
	for (std::map<hash_entry, ergc_neighborhood *>::iterator it = m_htable.begin(); it != m_htable.end(); ++it)
	{
...
void ERGCPktHashTable::Reset()
{
	m_htable.clear();
	/*
	  ergc_neighborhood *hashPtr;
	  Tcl_HashEntry *entryPtr;
	  Tcl_HashSearch searchPtr;

	  entryPtr = Tcl_FirstHashEntry(&m_htable, &searchPtr);
	  while (entryPtr != NULL) {
		  hashPtr = (ergc_neighborhood *)Tcl_GetHashValue(entryPtr);
		  delete hashPtr;
...
ergc_neighborhood *
ERGCPktHashTable::GetHash(AquaSimAddress senderAddr, unsigned int pk_num)
{
	hash_entry entry = std::make_pair(senderAddr, pk_num);
	std::map<hash_entry, ergc_neighborhood *>::iterator it;
	...
	unsigned int key[3];
	  ...
	  return (ergc_neighborhood *)Tcl_GetHashValue(entryPtr);
	*/
}

void ERGCPktHashTable::PutInHash(AquaSimAddress sAddr, unsigned int pkNum)
{
	// Tcl_HashEntry *entryPtr;
	...
	hashPtr = new ergc_neighborhood[1];
	hashPtr[0].number = 1;
	hashPtr[0].neighbor[0].x = 0;
	hashPtr[0].neighbor[0].y = 0;
	hashPtr[0].neighbor[0].z = 0;
	std::pair<hash_entry, ergc_neighborhood *> newPair;
	newPair.first = entry;
...
void ERGCPktHashTable::PutInHash(AquaSimAddress sAddr, unsigned int pkNum, Vector p)
{
...
int *DataHashTable::GetHash(int *attr)
{
...
void DataHashTable::PutInHash(int *attr)
{
	// bool newPtr = true;
...
bool ERGCRouting::Recv(Ptr<Packet> packet, const Address &dest, uint16_t protocolNumber)
{
	NS_LOG_FUNCTION(this);
	AquaSimHeader ash;
	VBHeader vbh;
	...
	{ // no headers //TODO create specalized Application instead of using this hack.
	...
	ergc_neighborhood *hashPtr = PktTable.GetHash(vbh.GetSenderAddr(), vbh.GetPkNum());
...
void ERGCRouting::ConsiderNew(Ptr<Packet> pkt)
{
	NS_LOG_FUNCTION(this);
	AquaSimHeader ash;
	VBHeader vbh;
	...
	case TARGET_DISCOVERY:
		...
		if (GetNetDevice()->GetAddress() == vbh.GetTargetAddr())
		{
			...
			// CalculatePosition(pkt); not necessary with mobilitymodel
			DataForSink(pkt);
			NS_LOG_INFO("ERGCRouting::ConsiderNew: target is " << GetNetDevice()->GetAddress());
	...
	case AS_DATA:
		NS_LOG_INFO("ERGCRouting::ConsiderNew: data packet");
...
void ERGCRouting::Terminate()
{
	NS_LOG_DEBUG("ERGCRouting::Terminate: Node=" << GetNetDevice()->GetAddress() << ": remaining energy=" << GetNetDevice()->EnergyModel()->GetEnergy() << ", initial energy=" << GetNetDevice()->EnergyModel()->GetInitialEnergy());
}

void ERGCRouting::StopSource()
{
	/*
	   Agent_List *cur;
...
Ptr<Packet>
ERGCRouting::PrepareMessage(unsigned int dtype,
...
void ERGCRouting::MACprepare(Ptr<Packet> pkt)
{
	AquaSimHeader ash;
	VBHeader vbh;
	pkt->RemoveHeader(ash);
...
void ERGCRouting::MACsend(Ptr<Packet> pkt, double delay)
{
...
void ERGCRouting::DataForSink(Ptr<Packet> pkt)
{
	if (!SendUp(pkt))
		NS_LOG_WARN("DataForSink: Something went wrong when passing packet up to dmux.");
}

void ERGCRouting::SetDelayTimer(Ptr<Packet> pkt, double c)
{
	NS_LOG_FUNCTION(this << c);
	if (c < 0)
		c = 0;
	Simulator::Schedule(Seconds(c), &ERGCRouting::Timeout, this, pkt);
}

void ERGCRouting::Timeout(Ptr<Packet> pkt)
{
	VBHeader vbh;
	AquaSimHeader ash;
	pkt->RemoveHeader(ash);
...
void ERGCRouting::CalculatePosition(Ptr<Packet> pkt)
{
	// not used.
	VBHeader vbh;
	AquaSimHeader ash;
...
double
ERGCRouting::CalculateDelay(Ptr<Packet> pkt, Vector *p1)
{
	VBHeader vbh;
	AquaSimHeader ash;
	pkt->RemoveHeader(ash);
...
double
ERGCRouting::Distance(Ptr<Packet> pkt)
{
	VBHeader vbh;
	AquaSimHeader ash;
	pkt->RemoveHeader(ash);
...
double
ERGCRouting::Advance(Ptr<Packet> pkt)
{
	VBHeader vbh;
	AquaSimHeader ash;
	pkt->RemoveHeader(ash);
...
double
ERGCRouting::Projection(Ptr<Packet> pkt)
{
	VBHeader vbh;
	AquaSimHeader ash;
	pkt->RemoveHeader(ash);
...
bool ERGCRouting::IsTarget(Ptr<Packet> pkt)
{
	VBHeader vbh;
	AquaSimHeader ash;
	pkt->RemoveHeader(ash);
...
bool ERGCRouting::IsCloseEnough(Ptr<Packet> pkt)
{
	// VBHeader vbh;
	// AquaSimHeader ash;
	// pkt->RemoveHeader(ash);
...
void ERGCRouting::DoDispose()
{
	m_rand = 0;
	AquaSimRouting::DoDispose();
}

void ERGCRouting::SetTargetPos(Vector pos)
{
	m_targetPos = pos;
}
