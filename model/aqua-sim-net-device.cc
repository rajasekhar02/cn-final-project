...
void AquaSimNetDevice::SetMac(Ptr<AquaSimMac> mac, Ptr<AquaSimSync> sync, Ptr<AquaSimLocalization> loc)
{
  // currently only supporting single layer per net device
  if (m_mac == 0)
  {
...
bool AquaSimNetDevice::SendWithHeader(Ptr<Packet> packet, uint16_t protocolNumber)
{
  AquaSimHeader ash;
  ...
  else if (m_phy)
  {
    ...
    NS_LOG_WARN("Routing/Mac/Phy layers are not attached to this device. Can not send.");
  return false;
}

bool AquaSimNetDevice::Send(Ptr<Packet> packet, const Address &dest, uint16_t protocolNumber)
{
...
bool AquaSimNetDevice::SendFrom(Ptr<Packet> packet, const Address &source, const Address &dest, uint16_t protocolNumber)
{
...
void AquaSimNetDevice::Receive(Ptr<Packet> packet, uint16_t protocol,
                               AquaSimAddress to, AquaSimAddress from)
{
