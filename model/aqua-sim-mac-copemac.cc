...
RevQueues::ClearExpired(Time ExpireTime)
{
  ...
  while( Head_ != NULL && Head_->EndTime < ExpireTime + Time(BACKOFF_DELAY_ERROR) ) {
      tmp = Head_;
      Head_ = Head_->next;
      delete tmp;
  }
